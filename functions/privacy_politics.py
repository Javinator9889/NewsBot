...
def policy(bot, update):
    ...
    if path.exists("first_run_{}".format(chat_id)):
        ...
        """bot.sendMessage(chat_id, "ᴛéʀᴍɪɴᴏs ᴅᴇ sᴇʀᴠɪᴄɪᴏ ʏ ᴘᴏʟíᴛɪᴄᴀs ᴅᴇ ᴘʀɪᴠᴀᴄɪᴅᴀᴅ\n\nAl utilizar este bot, *aceptas que recopilemos* y guardemos datos sobre ti tales como\
 tu *nombre de usuario*, *zona horaria*, *gustos y preferencias*. Dichos datos no son ni serán *compartidos con terceros* y estarán bajo protección *inclusive\
 cuando el usuario elimine* y *bloquee al bot*. En este caso, se pasará o a la *conservación de los datos* o a la *eliminación de los mismos*, según se convenga.\
          \nCuando el bot recibe una ubicación, por *motivos de seguridad*, primero hay que activar la opción de cambio de zona horaria para evitar cualquier *fuga de datos*.\
 A su vez, la ubicación *solo y únicamente se emplea* para determinar tu zona horaria. Después, *la ubicación enviada se elimina*.\n\nPara cualquier duda, contacta con @Javinator_9889 (creador)",
